	...
	<div class="grCertificate">
		...
		<div class="certificate" :class="inMoveAnimation ? 'aniTop' : ''" @animationend="moveEnd">
			...
			<div class="certificate-star">
				<p>光荣星级:</p>
				<ul>
					<li class="star" v-for="(item, index) in isIcon" :class="inAnimation == index ? 'anim' : ''" @animationend="inAnimation = 6"
					 :key="index" :style="{ backgroundImage: 'url(./static/' + (item ? 'icon1' : 'icon2') + '.png)' }"></li>
				</ul>
				<ul>
					<li class="star" v-for="(item, index) in isIcon" :key="index" :style="{ backgroundImage: 'url(./static/' + (item ? 'icon1' : 'icon2') + '.png)' }"></li>
				</ul>
			</div>
